...
namespace Utility
{
    ...
    {
        //public static string GetIp(HttpRequestBase pi_request)
        //{
        //    var ip = string.Empty;
        //    try
        //    {
        //        if (!string.IsNullOrEmpty(pi_request.ServerVariables["HTTP_VIA"]))
        //            ip = Convert.ToString(pi_request.ServerVariables["HTTP_X_FORWARDED_FOR"]);
        //        if (string.IsNullOrEmpty(ip))
        //            ip = Convert.ToString(pi_request.ServerVariables["REMOTE_ADDR"]);
        //    }
        //    catch { }
        ...
        {
            var code = Encoding.GetEncoding("gb2312");//定义文字编码 UTF-8
       ...
       'A','B','C','D','E','F','G','H','J','K', 'L','M','N','P','R','S','T','W','X','Y'
        ...
        {
            return getDictionaryData(data, ";");
        }

        //解析Dict
        public static string getDictionaryData(Dictionary<string, object> data, string splitChar)
        {
            string ret = null;
            foreach (var item in data)
            {
                ...
                {
                    ret += item.Key + "=" + (item.Value?.ToString() ?? "null") + splitChar;
                }
            }
            return ret;
        }
        ...
        //解析Dict
        public static string getSortedDictionaryData(SortedDictionary<string, object> data, string splitChar)
        {
            string ret = null;
            foreach (var item in data)
            {
                if (item.Value != null && item.Value.GetType() == typeof(SortedDictionary<string, object>))
                {
                    ret += item.Key + "={";
                    ret += getSortedDictionaryData((SortedDictionary<string, object>)item.Value, splitChar);
                    ret += "};";
                }
                else if (item.Value != null && item.Value.GetType() == typeof(Dictionary<string, object>))
                {
                    ret += item.Key + "={";
                    ret += getDictionaryData((Dictionary<string, object>)item.Value, splitChar);
                    ret += "};";
                }
                else
                {
        ...
        public static string getStringListData(List<string> list)
        {
            string ret = null;
        ...
        //生成N位随机数
        public static string getRandomNumberWithCount(int numberCount)
        {
            var sCode = string.Empty;
            char[] oCharacter = { '2', '3', '4', '5', '6', '8', '9', '1', '0' };
            var oRnd = new Random();
            for (var N1 = 0; N1 <= (numberCount - 1); N1++)
            {
                sCode += oCharacter[oRnd.Next(oCharacter.Length)];
            }
            return sCode;
        ...
        /// <param name="codeCount"></param>
        /// <returns></returns>
        public static string CreateRandomCode2(int codeCount)
        {
        ...
        public static string GetRamdomMobilePhone()
        {
            var rnd = new Random(StaticSeed.Seed);
            return "135" + string.Format("{0:00000000}", rnd.Next(0, 99999999));
        }
        ...
        #region 判断当前用户环境是PC端还是WAP端
        /// <summary>
        /// 判断当前用户环境是PC端还是WAP端 true WAP端  false PC端
        /// </summary>
        /// <param name="HTTP_USER_AGENT">HTTP_USER_AGENT = Request.ServerVariables["HTTP_USER_AGENT"];</param>
        /// <returns>
        /// true WAP端
        /// false PC端
        /// </returns>
        public static bool IsMobileBrowser(string HTTP_USER_AGENT)
        {
        ...
        public static int calculateAgeCorrect(DateTime birthDate, DateTime now)
        {
            ...
            var tmp = birthDate.AddYears(age);
        ...
        /// <summary>
        /// 将Object中为String类型的属性，转换为Dictionary，其他类型属性不做反应
        /// </summary>
        /// <param name="pi_obj"></param>
        /// <returns></returns>
        public static Dictionary<string, string> GetStringDicByClass(object pi_obj)
        {

            var classProperties = pi_obj.GetType().GetProperties();

            var properties = TypeDescriptor.GetProperties(pi_obj);

            var returnDic = new Dictionary<string, string>();

            foreach (PropertyDescriptor property in properties)
            {
                if (property.PropertyType.Name == "String" && property.GetValue(pi_obj) != null)
                    returnDic.Add(property.Name, property.GetValue(pi_obj).ToString());
            }
        ...
        /// <param name="pi_obj"></param>
        /// <returns></returns>
        public static SortedDictionary<string, string> GetStringSortedDicByClass(object pi_obj)
        {
            ...
            var returnDic = new SortedDictionary<string, string>();
        ...
        ///// 2个对象属性值进行呼唤
        ///// </summary>
        ///// <typeparam name="T">被进行赋值的对象</typeparam>
        ///// <param name="obj">进行赋值的对象</param>
        ///// <returns>返回实例化并且进行赋值了得T</returns>
        //public static T ConvertObjct<T>(object obj) where T : new()
        //{
        //    var model = new T();
        //    return (T)ConvertObjct(obj, model);
        //}
        ...
        ///// </summary>
        ///// <param name="obj">进行赋值的对象</param>
        ///// <param name="model">被进行赋值的对象</param>
        ///// <returns>返回实例化并且进行赋值了得model</returns>
        //public static object ConvertObjct(object obj, object model)
        //{
        //    var propertyInfos = model.GetType().GetProperties().ToList();
        //    var mapAttributes = obj.GetType().GetProperties().Where(f => f.GetCustomAttributes(typeof(ConvertAttribute), true).Length > 0).ToList();
        //    foreach (var mapAttribute in mapAttributes)
        //    {
        //        if (mapAttribute != null)
        //        {
        //            var name = mapAttribute.Name;
        //            var propertyInfo = propertyInfos.FirstOrDefault(f => f.Name == name);
        //            //现在要获取baseInfo对应的属性值
        //            var info = mapAttribute.GetValue(obj, null);
        //            propertyInfo?.SetValue(model, Convert.ChangeType(info, propertyInfo.PropertyType), null);
        //        }
        //    }
        //    return model;
        //}
