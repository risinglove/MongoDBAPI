...
namespace DAL
{
    public class CurrencyDAL<T> where T : new()
    {
        //private const string TableName = "LoginTable";
        private static MongoCollection<T> mc = null;
        public CurrencyDAL(string TableName)
        {
            using (DbHelper db = new DbHelper())
            {
                mc = db.DB.GetCollection<T>(TableName);
            }
        }
        ...
        public List<T> GetALL()
        {
            try
            {
                List<T> list = mc.FindAllAs<T>().ToList();
                return list;
            }
            catch (Exception ex)
            {
        ...
        /// 根据ObjectID 查询
        /// </summary>
        public T SelectOne(ObjectId objId)
        {
